...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'dietcode-opencode-'));
process.env.XDG_CONFIG_HOME = tmp;
delete process.env.DIETCODE_DEFAULT_MODE;
const statePath = path.join(tmp, 'opencode', '.dietcode-active');

let loadPlugin;
test.before(async () => {
  const url = pathToFileURL(path.join(__dirname, '..', '.opencode', 'plugins', 'dietcode.mjs'));
...
test('system.transform injects the ruleset at the default mode (full)', async () => {
  ...
  assert.match(system[0], /DIETCODE MODE ACTIVE — level: full/);
  assert.match(system[0], /lazy senior developer/);
});

test('command.execute.before persists /dietcode ultra, transform follows it', async () => {
  const hooks = await loadPlugin({});
  await hooks['command.execute.before']({ command: 'dietcode', arguments: 'ultra', sessionID: 's' });
  assert.equal(fs.readFileSync(statePath, 'utf8'), 'ultra');
  const system = await transform(hooks);
  assert.match(system[0], /DIETCODE MODE ACTIVE — level: ultra/);
});

test('/dietcode off persists off and transform injects nothing', async () => {
  const hooks = await loadPlugin({});
  await hooks['command.execute.before']({ command: 'dietcode', arguments: 'off', sessionID: 's' });
