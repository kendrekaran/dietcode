...
// always-on context, commands/*.toml for /dietcode + /dietcode-review, and
...
// carries the load-bearing rules — i.e. if the adapter stops wiring dietcode.
...
const REUSED_COMMANDS = ['commands/dietcode.toml', 'commands/dietcode-review.toml'];
const REUSED_SKILLS = ['skills/dietcode/SKILL.md'];
// Gemini CLI auto-loads this exact path for extension hooks. DietCode's
...
test('manifest names the dietcode extension with a pinned version', () => {
...
test('contextFileName resolves to a file carrying the dietcode rules', () => {
  const manifest = loadManifest();
