...
// format dietcode already uses, with one difference: `description` must be a
// single line under 160 chars. The canonical descriptions are long (tuned for
// the skill picker), so each ships a short one here. The body is copied
...
const HOMEPAGE = 'https://github.com/kendrekaran/dietcode';

const DESCRIPTIONS = {
  'dietcode': 'Lazy senior dev mode. Forces the simplest, shortest solution that works: YAGNI, stdlib first, no unrequested abstractions.',
  'dietcode-review': 'Review a diff for over-engineering. Finds what to delete: reinvented stdlib, needless deps, speculative abstractions. One line per finding.',
  'dietcode-audit': 'Audit the whole repo for over-engineering. A ranked list of what to delete, simplify, or replace with stdlib or native features.',
  'dietcode-debt': 'Harvest every dietcode: shortcut comment into one debt ledger, so deferrals get tracked instead of forgotten. One-shot report.',
  'dietcode-gain': 'Show dietcode measured impact as a scoreboard: less code, less cost, more speed, from the benchmark medians. One-shot display.',
  'dietcode-help': "Quick reference for dietcode's modes, skills, and commands. One-shot display.",
};
