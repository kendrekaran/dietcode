...
const copies = [
  ['.cursor/rules/dietcode.mdc', stripFrontmatter],
  ['.windsurf/rules/dietcode.md', text => text.trim()],
  ['.clinerules/dietcode.md', text => text.trim()],
  ['.agents/rules/dietcode.md', text => text.trim()],
  ['.github/copilot-instructions.md', text => text.trim()],
  ['.kiro/steering/dietcode.md', stripFrontmatter],
];
...
// so it cannot be byte-compared. dietcode: canary, not full equality. Assert the
...
const INVARIANTS = [
  ...
  // the four "stay rigorous about" safety carve-outs: pin each so a reword in
  // either file can't silently drop one. These are the continuous substrings
...
const skill = read('skills/dietcode/SKILL.md');
const sources = [['skills/dietcode/SKILL.md', skill], ['AGENTS.md', agents]];
