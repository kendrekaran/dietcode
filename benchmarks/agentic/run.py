#!/usr/bin/env python3
"""Agentic, multi-file benchmark for DietCode.
...
over-engineering score is a later pass (judge.py).
  ...
  python run.py --rescore runs/<stamp>
      Recompute metrics + aggregate from kept workspaces. No API.

dietcode: the claude CLI is the harness (already installed, we run inside it). No SDK
...
ARMS = {
    ...
    "dietcode":      lambda: _skill("skills/dietcode/SKILL.md"),
...
# cache dir (--plugin-dir).
PLUGIN_ARMS = ("dietcode",)                   # arms activated via --plugin-dir (vs raw --append prompts)
PLUGIN_CACHE = Path.home() / ".claude" / "plugins" / "cache"

def _plugin_dir(name):
    """Resolve a plugin's cache dir portably. Order: env override -> latest version dir under
    ~/.claude/plugins/cache -> clear error. Resolved per-arm at use-site."""
...
# write the implementation and stop. Writing tests is still explicitly allowed, so DietCode's
# "leave a runnable check" discipline is not suppressed.
...
def code_stats(workdir: Path):
    """LOC over code-extension source files only. total_loc counts every non-blank line including
    comments and docstrings -- the bloat a vibe baseline produces. src_loc is code-only. Tests
    tracked separately, never as bloat."""
...
def git_diff_stats(workdir):
    ...
    base. Tests counted separately; lockfiles/generated files skipped."""
    _git(workdir, "add", "-A")
...
def _selftest_plugin_dir():
    ...
    fails = 0
    sentinel = "/tmp/dietcode-selftest-plugin-dir"
    os.environ["DIETCODE_PLUGIN_DIR"] = sentinel
    try:
        ok_env = _plugin_dir("dietcode") == sentinel
    finally:
        del os.environ["DIETCODE_PLUGIN_DIR"]
    print(f"{'ok ' if ok_env else 'XX '} plugin_dir   env  override honored")
    fails += 0 if ok_env else 1
    missing = "dietcode-does-not-exist-xyz"          # no env, no cache entry -> must sys.exit
    try:
...
def _tree_kill(proc):
    """Kill only this cell's process tree -- never a blanket kill that could take down the
    surrounding Claude Code session. Windows uses taskkill /T; POSIX kills the process group."""
    if os.name == "nt":
