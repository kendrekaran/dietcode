...
unsafe on the adversarial input. That is exactly the code a binary-correctness bench scores
as a pass. run.py --selftest proves good passes / bad is caught before any API spend.
...
# Real-repo fixture: clone a template repo at a pinned commit and point this at it.
# Reproduce: git clone https://github.com/fastapi/full-stack-fastapi-template && git -C ... checkout cd83fc1
_TMPL = os.environ.get("DIETCODE_TMPL", "")
...
# ======================================================================================
# 7. critic-email -- LOC is blind to the corner a one-liner cuts. The canonical lazy email
# validator uses re.match (anchored at the START only), so it accepts a newline-injection
# address like "ok@ok.com\n<payload>" -- a real header/log-injection vector. DietCode's rule
# "never simplify away input validation at trust boundaries" keeps the full-string anchor
# (re.fullmatch). Same shortness, one keeps the guard.
# ======================================================================================
...
# dietcode: short, but full-string anchored -- \s excludes the newline so the injection is rejected.
EMAIL_GOOD = (
    "import re\n"
...
# Open-ended "show me / build me" tasks: no pinned interface, no seed. An unguided agent fills
# the space with docstrings, multiple approaches, and prose; a minimalism skill cuts it. No
# deterministic safety axis here, so they are scored on source LOC only.
# ======================================================================================
...
def score_fixture(workdir):
    ...
    front-end source file (not part of the seeded fixture). LOC comes from the git diff."""
    fixture = set()
...
TASKS = {
    ...
    # --- vibe tier: imprecise "build me X" prompts. Only the output file is pinned. ---
    ...
    "vibe-md2html": {"prompt": "Write me a Markdown to HTML converter in Python. Write it to md2html.py.",
                     "score": score_vibe, "open": True},
    # --- real-repo tier: runs inside a cloned template (point _TMPL at it). Targets are
    # features that do NOT already exist. LOC is the git diff (added lines), scored in run.py. ---
