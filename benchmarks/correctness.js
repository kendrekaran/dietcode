...
function extractBlocks(text) {
  ...
  // dietcode: terse models often answer with bare, unfenced code. Treat the whole
...
// dietcode: probe once at load; macOS and many Linux images ship python3 only.
let pythonCmd;
function python() {
...
function tmpFile(ext, content) {
  const p = path.join(os.tmpdir(), `dietcode-bench-${Date.now()}-${Math.random().toString(36).slice(2)}${ext}`);
